...
| `POST /gate` | 对已有 MinerU 解析目录跑质量门控，写出 `<stem>_gated.md` 并返回门控报告 |

`/parse`、`/parse_upload`、`/gate` 均做参数校验：缺失必填参数返回 HTTP 400，异常返回 HTTP 500（附错误类型与信息），正常返回 HTTP 200。完整字段说明见 [`docs/pdf_miner_agent_api.md`](docs/pdf_miner_agent_api.md)。

> `/gate` 暴露 `docgate.gate_and_rewrite`，使下游消费方能经 API 取得门控后的 markdown，而非进程内 import——`scripts/run_three_layer_ablation.sh` 的三层 RAG 消融正是通过此端点生成 `MinerU+GLM` 层，让消融成为封装 API 的真实端到端调用方。
...
`tests/test_pdf_miner_api.py` 共 10 个用例，覆盖核心类契约与四个端点（含 `/gate`）的正常/异常路径（400/500）：
...
pdf_miner_agent/
  ...
  __init__.py
tests/
  test_pdf_miner_api.py   # 10 个 pytest 可用性用例
docs/
